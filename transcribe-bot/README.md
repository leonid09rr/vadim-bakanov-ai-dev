# Transcribe Bot — транскрипция видео + умный чат

Telegram-бот: кидаешь видео или ссылку — получаешь три уровня разбора (темы → тезисы → детально) + полную транскрипцию файлом. **После разбора можно задать вопрос про видео** — бот помнит транскрипт и отвечает как собеседник, а не как справочник. Работает полностью на бесплатных лимитах Groq (Whisper + Llama 3.3 70B).
...
| [src/chat.py](src/chat.py) | Чат с транскриптом — память + ответы на вопросы юзера |
...
Длинные/тяжёлые видео — **только ссылкой**. Telegram Bot API не отдаёт боту файлы больше 20 МБ. Лайфхак: загрузи на **Яндекс.Диск** или **Google Drive** с публичной ссылкой — `yt-dlp` скачает оттуда без ограничений.

## Чат с видео

После того как бот выдал разбор, он помнит последний транскрипт **этого юзера** и готов на него отвечать. Просто напиши вопрос — без команд:

> _что главное в этом ролике?_
> _как это применить к моей ситуации?_
> _что автор имел в виду в 5-й минуте?_
> _с чем он не прав?_

Бот отвечает не как справочник, а как собеседник: думает, делает выводы, спорит, предлагает варианты. История последних 10 пар реплик помнится — можно задавать уточняющие ("разверни", "а почему?").

Память — в RAM, без БД. Сбрасывается при рестарте или новом видео. Принудительно — `/forget`.
...
| `/ask <вопрос>` | Спросить про последнее видео (или просто напиши текстом) |
| `/forget` | Забыть последнее сохранённое видео |
